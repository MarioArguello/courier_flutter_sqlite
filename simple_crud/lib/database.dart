...
class Courier extends TableElement {
  static final String TABLE_NAME = "courier";
  String title;

  Courier({this.title, id}) : super(id, TABLE_NAME);
  factory Courier.fromMap(Map<String, dynamic> map) {
    return Courier(title: map["title"], id: map["_id"]);
  }
...
class DatabaseHelper {
  ...
  Future<Database> open() async {
    try {
      ...
      var db = await openDatabase(path, version: 1,
        ...
        new Courier().createTable(database);
      });
      return db;
    } catch (e) {
      print(e.toString());
  ...
  Future<List<Courier>> getList() async {
    Database dbClient = await db;

    List<Map> maps =
        await dbClient.query(Courier.TABLE_NAME, columns: ["_id", "title"]);

    return maps.map((i) => Courier.fromMap(i)).toList();
  }
